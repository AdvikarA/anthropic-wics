...
export function ResultsDialog({ isOpen, onClose, results }: ResultsDialogProps) {
  if (!results) return null;
  
  // Extract scores for radar chart
  const categories = Object.keys(results).filter(key => key !== "engagementScore");
  const dataValues = categories.map(cat => {
    return typeof results[cat] === 'object' ? results[cat].score : results[cat];
  });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md md:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-center">Your Political Profile</DialogTitle>
        </DialogHeader>
